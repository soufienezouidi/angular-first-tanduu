...
export class SettingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userObject = JSON.parse(localStorage.getItem('rest'));
    this.userservices.getuserbyid(this.userObject.rest_id).subscribe((u) => {
      this.connecteduser = u.message;
      this.code_mobile = this.connecteduser.mobile
        ? this.getCurrentCodeCountry(this.connecteduser.mobile)
        : 'de';
      this.code_phone = this.connecteduser.phone
        ? this.getCurrentCodeCountry(this.connecteduser.phone)
        : 'de';
    });
  }
  ...
  selectFileuser(event: any) {
    ...
    formData.append(
      ...
      'user' + this.userObject.rest_id + '-' + 'profilepic' + '.png'
    );
    this.companyService
  ...
  getMobileCode(event: any) {
    this.connecteduser.code_mobile = this.getPhoneCountryCode(
  ...
  savechanges() {
    if (!this.connecteduser.first_name || !this.connecteduser.last_name) {
      Swal.fire(
        'Unable change profile informations',
        'First name and last name are required',
        'error'
      );
    } else {
      this.code_mobile = this.getCurrentCodeCountry(this.connecteduser.mobile);
      this.code_phone = this.getCurrentCodeCountry(this.connecteduser.phone);
      this.userservices.updateUser(this.connecteduser).subscribe((up) => {
        Swal.fire(
          'Profile updated',
  ...
  onsubmitSecurity() {
    if (this.newpassword !== this.repnewpassword) {
      ...
      Swal.fire(
        ...
        'Please verify your new password value',
        'error'
      );
    } else {
      let obj = {
      ...
      this.companyService.updateUserSecurity(obj).subscribe((data) => {
        ...
        if (!data.reset) {
          ...
          Swal.fire(
            'Unable to reset password?',
            'Please verify that you entred your old password ',
            'error'
          );
        } else {
          Swal.fire(
            'Passowrd updated',
            'use new password in your next login',
            'success'
          ).then((e) => {
            window.location.reload();
          });
