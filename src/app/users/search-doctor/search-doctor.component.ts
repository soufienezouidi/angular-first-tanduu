...
export class SearchDoctorComponent implements OnInit {
    ...
    if (this.ids.length > 0) {
      ...
      this.companyser.getAllcompaniesbyid(p).subscribe((results: any) => {
        ...
        this.listCompanies.forEach((puy) => {
          ...
          if(puy.category){        puy.category.languages = JSON.parse(puy.category.languages)}
  
          puy.socials = JSON.parse(puy.socials);
          puy.hashtags = JSON.parse(puy.hashtags).slice(0, 5);
        
          puy["url"]="https://api.aroundorder.com:1337/api/user/" + puy.userId
        +"/"+puy.id+"/logo.png"
        puy["cover"]="https://api.aroundorder.com:1337/api/user/" + puy.userId
        +"/"+puy.id+"/cover.png"
        puy['destination'] = {
          lat: puy.latitude,
          lng: puy.longitude,
          ...
          this.companyser.getserviesbycompany(puy.id).subscribe((ee) => {
            ...
            if(ee.services){
              ...
              JSON.parse(ee.services).forEach(e => {
                ...
                if(e.service){ e.service.languages = JSON.parse(e.service.languages)
                  arr.push(e)}
  ...
  gotoprofile(item) {
    this.router.navigate(['/', item.company_link]).then((e) => {
      window.location.reload();
    });
  }
}