...
export class CompanyChatService {
  ...
  getcompanyfilechat(ida: number): Observable<any> {
    var users: any = {
      id_company: ida,
    };
    return this.http
      .post<any>(
        UrlsService.backendUrl + '/api/chat_company/getfilename',
        users,
        this.httpOptions
      )
      .pipe(retry(1));
  }
  getchatgroupsbyuser(ida: number): Observable<any> {
    var users: any = {
      userId: ida,
    };
    return this.http
      .post<any>(
        UrlsService.backendUrl + '/api/chat_company/group/chat/identifier',
        users,
        this.httpOptions
      )
      .pipe(retry(1));
