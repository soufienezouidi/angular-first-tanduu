...
export class DemoAdapterPagedHistory
  extends PagedHistoryChatAdapter
  implements IChatGroupAdapter
{
  ...
  public getMessageHistoryByPage(
    destinataryId: any,
    size: number,
    page: number
  ): Observable<Message[]> {
    ...
    let mockedHistory: Array<Message> = this.historyMessages.slice(
      startPosition,
      endPosition
    );
  ...
  sendMessage(message: Message): void {
    setTimeout(() => {
      ...
      if (isNaN(message.toId)) {
        let group = DemoAdapter.mockedParticipants.find(
          (x) => x.id == message.toId
        ) as Group;

        // Message to a group. Pick up any participant for this
        let randomParticipantIndex = Math.floor(
          Math.random() * group.chattingTo.length
        );
        ...
        let user = DemoAdapter.mockedParticipants.find(
