...
export class Home1headerComponent implements OnInit {
  searchServiceMobHome1Header: any;
  languagesList: any[] = [
    /*  {
        lang: 'Arabic',
        symbol: 'ar',
  ...
  searchServices(value: any) {
    this.searchServiceMobHome1Header = value;
  ...
  selectServicemob(id: any, name: any, item: any) {
    ...
    $('input[name=header1mob]').focus();
    this.Finaleselectedservice = item;
    this.searchSeletedService = id;
    this.sendjoinreq(name);

    this.searchServiceMobHome1Header = name;
    this.showSuggestion = false;
  }
  gotodashbard() {
    if (this.cu.roles[0] === 'ROLE_ADMIN') {
  ...
  setupSocketConnection() {
    ...
    this.socket.on('message-from-search-engine-server', (data: any) => {
      ...
      if (this.newArr.length == 0) {
        let o: any = {
          text: this.searchServiceMobHome1Header,
  ...
  gotoresults() {
    ...
    Swal.fire({
      ...
      .navigate(['/search_results'], {
        queryParams: { search: this.searchServiceMobHome1Header },
      })
      .then(() => {
        window.location.reload();
      });
