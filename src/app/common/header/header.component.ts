...
export class HeaderComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.userconnected != null) {
      if (JSON.parse(this.userconnected).user) {
        ...
        if(JSON.parse(this.userconnected).roles[0]=== "ROLE_COMMERCIAL" && JSON.parse(this.userconnected).roles[0]=== "ROLE_TANDUU_ADMIN"){
  ...
  gogtosub(i) {
    this.router.navigate(['/cateogry', i.name]).then((e) => {
      window.location.reload();
    });
  }
  ...
  searchServicesmob(value: any) {
    this.searchServiceMobHeader = value;
    this.sendjoinreq(value);
    if (value) {
      this.showSuggestion = true;
      var sp = value.split(' ');
  ...
  setupSocketConnection() {
    ...
    this.socket.on('message-from-search-engine-server', (data: any) => {
      ...
      if (this.newArr.length == 0) {
        let o: any = {
          text: this.searchServiceMobHeader,
  ...
  gotoresults() {
    ...
    width : '250px',
      ...
      .navigate(['/search_results'], {
        queryParams: { search: this.searchServiceWebHeader1 },
      })
      .then(() => {
        window.location.reload();
      });
