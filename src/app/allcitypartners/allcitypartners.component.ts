...
@Component({
  selector: 'app-allcitypartners',
  templateUrl: './allcitypartners.component.html',
  styleUrls: ['./allcitypartners.component.css'],
})
export class AllcitypartnersComponent implements OnInit {
  slider;
  infowindow = null;
  ...
  ngOnInit(): void {
    ...
    this.companyser
      .gelallcitycompanies(this.searchLocationInfos.city)
      .subscribe((el: any) => {
        this.results = el;
        ...
        this.results.forEach((r: any) => {
          ...
          r.company.socials.lnk.link = 'https://' + r.company.socials.lnk.link;

          r.company.socials.twt.link = 'https://' + r.company.socials.twt.link;
          ...
          r.services.forEach((elementz) => {
            elementz.service.languages = JSON.parse(elementz.service.languages);
          });
  ...
  gotoprofile(item) {
    this.router.navigate(['/', item.user.username]).then((e) => {
      window.location.reload();
    });
  }
}
